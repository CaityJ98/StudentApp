...
router.get('/', async (req, res) => {
    ...
    try {
        ...
        if (search !== '' && search !== undefined) {
            ...
            listings = listings.filter(x => x.title.toLowerCase().includes(search.toLowerCase()) || x.city.toLowerCase().includes(search.toLowerCase()))
            res.status(200).json({ listings: listings, pages: listings.pages });
        } else {
            listings = await Listing.paginate({}, { page: parseInt(page) || 1, limit: 5 });
...
router.get('/:category', async (req, res) => {
    ...
    try {
        let listings = await Listing.paginate({ category: req.params.category }, { page: parseInt(page) || 1, limit: 10 });
        res.status(200).json({ listings: listings.docs, pages: listings.pages });
    } catch (error) {
        res.status(500).json({ message: error.message })
    }
...
router.patch('/edit/:id', isAuth, async (req, res) => {
    ...
    try {
        ...
        if (!condition || condition == "Choose...") errors.push('Condition is required; ');

        if (errors.length >= 1) throw { message: [errors] };

        if (req.body.image) {
            let compressedImg = await listingService.uploadImage(req.body.image);
            await listingService.edit(req.params.id, { title, price, description, condition, category, image: compressedImg });
        } else {
            await listingService.edit(req.params.id, { title, price, description, condition, category });
        }
