...
async function registerUser(userData) {
    ...
    if (university.length < 3 || university.length > 50) errors.push('Name should be at least 3 characters long and max 50 characters long; ')
...
async function loginUser({ email, password }) {
    ...
    let token = jwt.sign({ _id: user._id, email: user.email, university: user.university, createdSells: user.createdSells.length, avatar: user.avatar }, SECRET);
    return token;
}
